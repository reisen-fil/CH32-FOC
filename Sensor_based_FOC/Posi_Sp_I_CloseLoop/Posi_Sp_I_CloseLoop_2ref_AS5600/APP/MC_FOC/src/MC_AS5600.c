...
void EncoderAlign_UpdateAngle(ENCODER_PARAM_T *encoder_pHandle)
{
    ...
    mechAngleNorm = NORM_ENCODER(mechAngleRaw);
...
{
    return _IQmpy(normAngle, PI_2);  // 2 * PI
}

_iq EncoderAlign_NormToAngle(_iq normAngle)
{
    return _IQmpy(normAngle, Angle_PI_2);  // 2 * PI
}
