...
void PMSM_ParaIdentify_Function(PARAM_IDENTIFY_T *PMSM_phandle, MC_FOC_SYSTEM_T *foc_phandle)
{
	...
	{
		case PARAM_IDLE:
			// PMSM_phandle->state = PARAM_MEASURE_R_1;
            break;
		...
		case PARAM_MEASURE_Ld:
			if(PMSM_phandle->steady_time_cnt < 60) 
			...
			else
			{
				...
				FOC_Ctrl(_IQ(0.0f),_IQmpy(_IQ(1.38f),SIN_TABLE_12F[PMSM_phandle->sample_count]),foc_phandle);	/* Injected voltage modulation ratio is 0.2 */	
				PMSM_phandle->sample_count++;
				...
				switch(PMSM_phandle->identify_state)		
				{
					case 0:		/* Wait for the current response to stabilize */
						...
						if(PMSM_phandle->state_timer >= 6000)	/* After injecting for 1.0s */
						{
							PMSM_phandle->state_timer = 0;
					...
					case 1:		/* Start acquiring the response current */
						...
						PMSM_phandle->state_timer++;
						if(PMSM_phandle->state_timer>120 && PMSM_phandle->state_timer<=248)	/* Sample 128 data points */
						{
							PMSM_phandle->i[PMSM_phandle->state_timer-121].Re = foc_phandle->current_sample.I_d;
							PMSM_phandle->i[PMSM_phandle->state_timer-121].Im = _IQ(0.0);
						}
						else if(PMSM_phandle->state_timer>248) /* After sampling, perform FFT to extract the amplitude */
						{
							...
							PMSM_phandle->Ld = _IQdiv(_IQmpy(MC_Modulate_Ref,_IQ(0.2)),_IQmpy(PMSM_phandle->rfft_I_amp,_IQmpy(_IQ(FFT_inject_Freq),PI_2)));
		...
		case PARAM_MEASURE_Lq:
			...
			{
				/* Inject high-frequency sine signal (1kHz). With a 12kHz execution rate, one period is divided into 12 steps */
				FOC_Ctrl(_IQmpy(_IQ(1.38f),SIN_TABLE_12F[PMSM_phandle->sample_count]),_IQ(0.0f),foc_phandle);	/* Injected voltage modulation ratio is 0.2 */	
				PMSM_phandle->sample_count++;
				if(PMSM_phandle->sample_count >= 11) PMSM_phandle->sample_count = 0;
				...
				switch(PMSM_phandle->identify_state)		
				{
					...
					case 1:		/* Start acquiring the response current */
						...
						if(PMSM_phandle->state_timer>120 && PMSM_phandle->state_timer<=248)	/* Sample 128 data points */
						{
							PMSM_phandle->i[PMSM_phandle->state_timer-121].Re = foc_phandle->current_sample.I_q;
							PMSM_phandle->i[PMSM_phandle->state_timer-121].Im = _IQ(0.0);
						}
						else if(PMSM_phandle->state_timer>248) /* After sampling, perform FFT to extract the amplitude */
						{
                            ...
                            /* Calculate Lq using V = L * di/dt -> L = V / (2 * pi * f * I) */
							PMSM_phandle->Lq = _IQdiv(_IQmpy(MC_Modulate_Ref,_IQ(0.2)),_IQmpy(PMSM_phandle->rfft_I_amp,_IQmpy(_IQ(FFT_inject_Freq),PI_2)));
