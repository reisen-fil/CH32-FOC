...
void MC_FOC_State_Init(MC_FOC_SYSTEM_T *foc_phandle)
{
    ...
    MC_PI_Init(&foc_phandle->Po_control, _IQ(Position_Ctrl_kp),0.0,0.0,MC_POpi_Out_Limit);
...
static void FOC_Model_Callback(MC_FOC_SYSTEM_T *foc_phandle,ENCODER_PARAM_T *encoder_pHandle)
{
    ...
    MC_PI_CONTROLLER_T *pPoCtrl = &foc_phandle->Po_control;
    ...
    switch (pState->current_state) {
        ...
        case FOC_STATE_STOP:
        {
            ...
            if (Target_Po_test >= _IQ(0.05)) {  /* Startup request exists */
        ...
        {    
            ...
            {
                if(Target_Po_test >= _IQ(350.0)) Target_Po_test = _IQ(350.0);         /* Set angle control deadband */
                else if(Target_Po_test <= _IQ(5.0)) Target_Po_test = _IQ(5.0);                

                pPoCtrl->now = EncoderAlign_NormToAngle(EncoderAlign_GetMechanicalAngle(encoder_pHandle));                   
                MC_P_Calculate(pPoCtrl,Target_Po_test);                
                if(_IQabs(pPoCtrl->out) <= _IQ(0.07))  pPoCtrl->out = 0.0;   /* Force target speed to 0 when the motor position stabilizes to eliminate steady-state jitter */ 
                ...
                MC_PI_Calculate(pSpCtrl,pPoCtrl->out);
                I_Ctrl_cnt = 0;                
            }
