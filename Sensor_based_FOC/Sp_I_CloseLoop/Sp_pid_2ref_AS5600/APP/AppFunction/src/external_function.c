...
{
    uart_printf("I:%d,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%f,%f,%f,%d,%d,%d\n",
...
void CtrlModeSet_Handle(uint8_t* Buffer)
{
    #if(MC_CM_Select == 1)
    {
        if(!strncmp((char*)Buffer,"Target_Sp = ",12)) Target_Sp_test = RxPacket_Data_Handle(Buffer,12);
    }
    #elif(MC_CM_Select == 2)
    {
        if(!strncmp((char*)Buffer,"Sp=",3)) Target_Sp_test = RxPacket_Data_Handle(Buffer,3);
    }
    #endif
}
...
{    
    ...
    if(pFocState->current_state >= FOC_STATE_STANDBY)
    {
        ...
        {        
            ...
            {
                case DEVICE_IDLE:
                    if(!strcmp((char*)uart_rx_buffer->rxbuf,"SPEED_CTRL_MODE"))
                    {
                        pFocState->vofa_ctrlfoc_state = FOC_SPEED_CTRL_MODE;                    
                    ...
                    break;                                     
                case FOC_SPEED_CTRL_MODE:        
                    CtrlModeSet_Handle(uart_rx_buffer->rxbuf);      /* Enter host Speed control mode */
                    if(!strcmp((char*)uart_rx_buffer->rxbuf,"DEVICE_IDLE"))
                    {
                        ...
                        pFocState->current_state = FOC_STATE_STANDBY;       /* Speed closed-loop control state setting */                             
                    }
                    break;
        ...
        #elif(MC_CM_Select == 2)
        {
            ...
            {                
                case DEVICE_IDLE:
                    if(!strcmp((char*)can_rx_buffer,"SP_CTRL"))
                    {
                        pFocState->vofa_ctrlfoc_state = FOC_SPEED_CTRL_MODE;                    
