...
{
    uart_printf("I:%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%f,%f,%f,%f,%d,%d,%d\n",
...
{
    if(!strncmp((char*)Buffer,"Target_Sp = ",12)) Target_Sp_test = RxPacket_Data_Handle(Buffer);
...
{    
    ...
    if(pFocState->current_state >= FOC_STATE_STANDBY)
    {
        ...
        {
            case DEVICE_IDLE:
                if(!strcmp((char*)rx_buffer,"SPEED_CTRL_MODE"))
                {
